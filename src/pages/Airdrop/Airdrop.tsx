...
const GetButton = styled.button`
  ...
  padding: 5px 15px;
  border-radius: 10px;
  box-shadow: 2px 3px 3px rgba(228, 92, 49, 0.1);
  &:hover {
    cursor: pointer;
    background-color: rgba(126, 162, 36, 0.8);
  }
`
...
const Airdrop = () => {
  ...
  useEffect(() => {
    const init = async () => {
      const { lockperiod, userinfo, airdrops } = await GetAirdropInfo(account, library)
  ...
  const reducer = (previousValue, currentValue) =>
    new BigNumber(previousValue.amount).plus(new BigNumber(currentValue.amount))
      ...
      <Wrapper>
        ...
        {metrics && (
          <div className="row center" style={{ marginTop: 10, paddingBottom: 3 }}>
            <TextB align="center" color={Theme.colors.black}>
              {t('Claimed today')}: {metrics.result.elligible}
            </TextB>
            <SpaceH />
            <TextB align="center" color={Theme.colors.black}>
              {t('New Participants')}: {metrics.result.current_participants}
            </TextB>
            <SpaceH />
            <TextB align="center" color={Theme.colors.black}>
              {t('Total Participants')}: {metrics.result.participants}
            </TextB>
          </div>
        )}
        <Container>
          ...
          <StyledModal>
            ...
            {account && userStatus !== '' && (
              <TextB color="#FFF" align="center">
                {t('User Status')}: {t(`${userStatus}`)}
              </TextB>
            )}
            {account && userStatus === 'You are eligible.' && <Spacer height="10px" />}
            {account && userStatus === 'You are eligible.' && (
              <Text color="#FFF" align="center">
                {t(`Your ballance will be visible tomorow.`)}
              </Text>
            )}
            <Line />
            <GetButton onClick={() => handleLogin()}>
              <Text align="center" color={Theme.colors.white}>
                {account ? shortenAddress(account) : t('Connect Wallet')}
              </Text>
            </GetButton>
            <Spacer height="20px" />
            <Column>
              <FlexBetween>
                <div>
                  ...
                  >
                    <Text align="left" color={Theme.colors.white} weight="bold">
                      {t('Details:')}:
                    </Text>
                    {showDetail ? (
              ...
              {showDetail && account && (
                <DetailContent>
                  <RowBaseFlex>
                    ...
                    <TbDiv2>
                      <Text align="right" color={Theme.colors.white}>
                        {t('Claim Date')}
                      </Text>
                    </TbDiv2>
                  </RowBaseFlex>
                  {Airdrops?.map((item, index) => {
                    ...
                    const datetime = toDateTime(
                      parseInt(userInfo[index].timestamp._hex) + lockPeriod * oneDaySeconds,
                    ).toString()
                    const b =
                      parseInt(getBalanceAmount(new BigNumber(userInfo[index].amount._hex)).times(BIG_100).toString()) /
                      100
                    if (b === 0) return null
                    return (
            ...
            <Text align="left" color={Theme.colors.white}>
              {t(
                'The system checks your eligibility when you connect your wallet. Please refer to the Airdrop rules for details.',
              )}
            </Text>
            <Spacer height="20px" />
            ...
            <Spacer height="10px" />
            <Text align="left" color={Theme.colors.white}>
              &quot;{t('Balance is updated every 24 hours.')}&quot;
            </Text>
            <Spacer height="5px" />
            <Text align="left" color={Theme.colors.white}>
              &quot;{t('First Airdrop started on 15/10/2021.')}&quot;
            </Text>
            <Spacer height="20px" />
          </StyledModal>
          <Spacer height="100px" />
