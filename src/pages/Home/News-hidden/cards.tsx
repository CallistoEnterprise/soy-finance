  ...
  return (
    <StyledCard>
      <Container img={Assets.news2}>
        <StyledText fontWeight="bold" fontSize="30px" color="black">
          {t('Soy Staking')}
        </StyledText>
        <StyledText fontWeight="600" fontSize="21px" color="black">
          {t('Stake Soy, Earn SOY')}
        </StyledText>
        <Spacer height="30px" />
        <WhiteCard width="80%">
          <StyledImage src={Assets.token} alt="token" />
          <StyledText fontWeight="400" fontSize="16" color="black">
            {t(`APR: ${getShortenNumber(apr)}%`)}
          </StyledText>
        </WhiteCard>
      </Container>
    </StyledCard>
        ...
        {data?.map((item) => {
          ...
          return (
            <div key={item.name}>
              ...
              <WhiteCard>
                <StyledImage src={Assets.token} alt="token" />
                <StyledText fontWeight="bold" fontSize="16" color="black" marginRight="20px">
                  +
                </StyledText>
                <StyledImage src={logo} alt="token" />
                <div>
                  <StyledText fontWeight="bold" fontSize="16" color="black">
                    {t(`${tkName}`)}
                  </StyledText>
                  <StyledText fontWeight="400" fontSize="16" color="black">
                    {getShortenNumber(item.apr)}
                  </StyledText>
                </div>
              </WhiteCard>
            </div>
  ...
  return (
    <StyledCard>
      <Container img={Assets.news4}>
        <StyledText fontWeight="bold" fontSize="30px" color="white">
          {t('Total Token Burn')}
        </StyledText>
        <StyledText fontWeight="bold" fontSize="30px" color="white">
          {t(`${burnedSoy} $SOY`)}
